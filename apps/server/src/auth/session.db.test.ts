...
describe('displayNameFor', () => {
  it('uses the name when there is one', () => {
    const pubkey = getPublicKeyHex(generateSecretKey());
    assert.equal(
      displayNameFor({ userId: 'u', name: '  Ada  ', pubkey, isGuest: false, description: '', avatar: '', guestWorkspaceId: null }),
      'Ada',
    );
  });
  ...
  it('falls back to a truncated npub, not to a placeholder', () => {
    ...
    const name = displayNameFor({ userId: 'u', name: '', pubkey, isGuest: false, description: '', avatar: '', guestWorkspaceId: null });
  ...
  it('never returns an empty label, even for a nonsense key', () => {
    assert.equal(
      displayNameFor({ userId: 'u', name: '', pubkey: 'garbage', isGuest: false, description: '', avatar: '', guestWorkspaceId: null }),
      'Someone',
    );
  });
});
