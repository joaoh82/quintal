...
describe('a turn the runtime goes silent on', () => {
  ...
  const ENV = ['FAKE_DELAY_MS', 'QUINTAL_TURN_IDLE_MS'] as const;
  ...
  async function start(env: Record<(typeof ENV)[number], string>) {
    await stopCurrent();
  ...
  it('is stopped, and the person waiting is told why, where they asked', async () => {
    ...
    await until(() => said.some((s) => /said nothing for 1 minute/.test(s.text)), 'the stall notice');
